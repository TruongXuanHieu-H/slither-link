...
for i in range(1):
    ...
    def get_second_solver():
        return SlitherLinkAddAllLoopWithEmpty(get_second_solver_params())
    ...
    result_evaluation = "Good" if second_solver_time_elapsed[-1] < first_solver_time_elapsed[-1] else "Bad"
    output_file = (f"output/{result_evaluation}/"
                   f"{first_solver_name} VS "
